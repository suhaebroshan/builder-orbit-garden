      ...
      <div className="relative w-full h-full rounded-[2.4rem] border border-black/20 shadow-2xl overflow-hidden bg-black">
        <div
          className={cn(
            ...
            theme === "amoled" ? "bg-black" : "bg-[hsl(var(--background))]",
          )}
    ...
    <div className="h-6 text-[11px] text-white/90 flex items-center justify-between px-2">
      ...
      <div className="flex items-center gap-2">
        <span title="notifications">
          {notifications.filter((n) => n.unread !== false).length > 0
            ? "🔔"
            : ""}
        </span>
        <span title="network">
          {network.wifi ? "📶" : network.type !== "none" ? network.type : "✈️"}
        </span>
        <span title="battery">
    ...
    <div className="absolute inset-0" style={{ background: state.wallpaper }}>
      ...
      >
        <div
          ...
          style={{ transform: `translateY(${drag * -0.25}px)` }}
        >
  ...
  return (
    <div className="absolute inset-0">
      <div
        ...
        aria-hidden
        style={{ background: "var(--wallpaper, transparent)" }}
      />
      <StatusBar />
      <HomeScreen />
      <Notifications />
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 px-4">
        <div
          className={cn(
            "grid grid-cols-4 gap-3 transition-transform",
            edit && "scale-[0.98]",
          )}
        >
          {grid.map((app) => (
            ...
            >
              <div
                className={cn(
                  "h-12 w-12 grid place-items-center rounded-2xl bg-white/10 backdrop-blur border border-white/20 shadow-lg group-active:scale-95 transition",
                  edit && "animate-[pulse_2s_infinite]",
                )}
              >
                {app.icon}
              </div>
...
function NavBar() {
  ...
  return (
    <div className="absolute bottom-1 left-0 right-0 flex items-center justify-center gap-16 text-white/80">
      <button
        onClick={() => dispatch({ type: "CLOSE_TOP_APP" })}
        className="h-8 w-8 grid place-items-center rounded-full hover:bg-white/10 active:scale-95"
      >
        ◁
      </button>
      <div className="h-1 w-24 rounded-full bg-white/60" />
      <button
        onClick={() => dispatch({ type: "OPEN_APP", id: "playstore" })}
        className="h-8 w-8 grid place-items-center rounded-full hover:bg-white/10 active:scale-95"
      >
        ▢
      </button>
...
function OpenAppsLayer() {
  ...
  return (
    <div className="absolute inset-0">
      <AppWindow
        app={top}
        onClose={() => dispatch({ type: "CLOSE_TOP_APP" })}
      />
    </div>
  );
}
      ...
      <div className="absolute inset-2 rounded-2xl overflow-hidden shadow-2xl">
        <div className="h-10 px-3 flex items-center justify-between text-white/90 bg-black/40">
          <div className="flex items-center gap-3">
            <div className="h-7 w-7 grid place-items-center rounded-md bg-white/5 text-lg">
              {state.apps.find((a) => a.id === app.id)?.icon}
            </div>
            <div className="text-sm font-medium">
              {state.apps.find((a) => a.id === app.id)?.name}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
...
function Notifications({ shadeOnLock = false }: { shadeOnLock?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "absolute inset-x-0 top-0 transition-transform",
      ...
      <div className="bg-black/50 backdrop-blur px-3 pt-2 pb-3">
        ...
        <div className="flex flex-col gap-2 max-h-64 overflow-auto">
          ...
          {state.notifications.map((n) => (
            <div
              key={n.id}
              className="rounded-xl bg-white/10 p-2 text-white border border-white/10"
            >
              ...
              <div className="mt-1 flex gap-2">
                ...
                <button
                  ...
                  className="text-xs px-2 py-1 rounded bg-white/20 ml-auto"
                >
                  Dismiss
                </button>
              </div>
      ...
      <div className="bg-black/70 backdrop-blur px-3 pt-3 pb-4 text-white">
        ...
        <div className="grid grid-cols-3 gap-2">
          {(
            ...
            <button
              ...
              className={cn(
                "rounded-xl py-2 text-sm border border-white/10",
                state.quickSettings[key] ? "bg-white/20" : "bg-white/5",
              )}
            >
              {label}
            </button>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 items-center text-xs">
          ...
          <input
            ...
            value={state.volume}
            onChange={(e) =>
              dispatch({ type: "SET_VOLUME", value: Number(e.target.value) })
            }
          />
        </div>
      </div>
...
function CalculatorApp() {
  ...
  return (
    <div className="h-full flex flex-col p-3 bg-neutral-900 text-white">
      ...
      <div className="text-right text-lg min-h-[2rem] opacity-70">
        {res ?? ""}
      </div>
      <div className="mt-3 grid grid-cols-4 gap-2">
        <button
          onClick={clearAll}
          className="col-span-2 py-3 rounded-xl bg-white/10"
        >
          AC
        </button>
        <button
        ...
        {keys.map((k) =>
          k === "=" ? (
            <button
              key={k}
              onClick={evalExpr}
              className="py-3 rounded-xl bg-emerald-500 text-black col-span-1"
            >
              =
            </button>
          ) : (
...
function ClockApp() {
  ...
  useEffect(() => {
    if (!running) return;
    const i = setInterval(() => setMs((m) => m + 100), 100);
  ...
  return (
    <div className="h-full relative bg-black">
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-center">
        <button
          onClick={capture}
          className="h-14 w-14 rounded-full bg-white/90"
        />
      </div>
      {!ready && (
...
function GalleryApp() {
  ...
  useEffect(() => {
    try {
      setPhotos(JSON.parse(localStorage.getItem("emu_photos") || "[]"));
    } catch {}
  }, []);
  return (
    ...
    <div className="h-full p-4 space-y-4 overflow-auto bg-neutral-900 text-white">
      <div>
        ...
        <div className="flex gap-2">
          {["light", "dark", "amoled"].map((t) => (
            <button
              key={t}
              onClick={() => dispatch({ type: "SET_THEME", value: t as any })}
              className={cn(
                "px-3 py-2 rounded border",
                state.theme === t ? "bg-white text-black" : "bg-white/10",
              )}
            >
              {t}
            </button>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <button
              key={css}
              onClick={() => dispatch({ type: "SET_WALLPAPER", css })}
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              dispatch({ type: "SET_BATTERY", level: Number(e.target.value) })
            }
          />
      ...
      <div>
        ...
        <div className="text-sm opacity-80">
          Web Android Emulator — experimental build
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-full p-4 bg-neutral-900 text-white flex flex-col">
      <div className="text-center text-2xl h-10">
        {active ? `In call: ${digits}` : digits}
      </div>
      {active ? (
        <div className="flex-1 grid place-items-center text-5xl">
        ...
        <div className="grid grid-cols-3 gap-3 mt-6">
          {nums.map((n) => (
            <button
              key={n}
              onClick={() => press(n)}
              className="rounded-full aspect-square bg-white/10 text-xl"
            >
              {n}
            </button>
          ))}
      ...
      <div className="mt-auto flex justify-center gap-4">
        {!active ? (
          ...
          <button
            onClick={hang}
            className="px-6 py-2 rounded-full bg-rose-500 text-black"
          >
            Hang up
          </button>
        )}
...
function MessagesApp() {
  ...
  function send() {
    if (!text.trim()) return;
    const msg = { me: true, t: Date.now(), body: text };
    setLog((l) => [...l, msg]);
    setText("");
    setTimeout(
      () =>
        setLog((l) => [
          ...l,
          { me: false, t: Date.now(), body: "Auto‑reply: " + msg.body },
        ]),
      800,
    );
  }
    ...
    <div className="h-full flex flex-col bg-neutral-900 text-white">
      ...
      <div className="p-2 flex gap-2 border-t border-white/10">
        <input
          ...
          placeholder="Type…"
          className="flex-1 bg-white/10 rounded px-3 py-2 outline-none"
        />
        <button onClick={send} className="px-3 rounded bg-white/20">
          Send
        </button>
      </div>
    </div>
...
function ContactsApp() {
  const [contacts, setContacts] = useState<{ name: string; phone: string }[]>([
    { name: "Demo", phone: "555-0100" },
  ]);
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");
  function add() {
    if (!name || !phone) return;
    setContacts((c) => [...c, { name, phone }]);
    setName("");
    setPhone("");
  }
  return (
    <div className="h-full p-4 bg-neutral-900 text-white">
      ...
      <div className="flex gap-2 mb-3">
        ...
        <input
          ...
          className="bg-white/10 rounded px-2 py-1"
        />
        <button onClick={add} className="px-3 rounded bg-white/20">
          Add
        </button>
      </div>
      <ul className="space-y-1 text-sm">
